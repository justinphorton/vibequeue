...
from .models import SpotifyToken, Song, Message
...
class CurrentPlayback(APIView):
    def get(self, request, format=None):
        ...
        code, current_data = spotify_api_request(session_key, "GET", "/me/player/currently-playing")

        # Get queue (includes currently playing and next tracks)
        queue_code, queue_data = spotify_api_request(session_key, "GET", "/me/player/queue")
        ...
        if isinstance(current_data, dict) and current_data.get("item"):
            track = current_data["item"]
            response_data["current_track"] = {
                "name": track.get("name"),
                "artist": track["artists"][0]["name"] if track.get("artists") else "Unknown",
                "image": track["album"]["images"][0]["url"] if track.get("album", {}).get("images") else None
            }
            response_data["is_playing"] = current_data.get("is_playing", False)

        # Get next track from queue
        if isinstance(queue_data, dict) and queue_data.get("queue") and len(queue_data["queue"]) > 0:
            next_track = queue_data["queue"][0]
            response_data["next_track"] = {
                "name": next_track.get("name"),
                "artist": next_track["artists"][0]["name"] if next_track.get("artists") else "Unknown"
            }
...
class SaveManualSong(APIView):
    def post(self, request, format=None):
        ...
        uri = (request.data.get("uri") or "").strip()
